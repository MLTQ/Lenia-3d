use crate::{KernelCore, KernelMode, LeniaParams, Real};
use ndarray::Array3;
...
pub fn generate_kernel_3d(params: &LeniaParams) -> Kernel3D {
    match params.kernel_mode {
        ...
        KernelMode::LeniaBands => generate_lenia_band_kernel_3d(params),
    }
}
...
fn generate_lenia_band_kernel_3d(params: &LeniaParams) -> Kernel3D {
    let diameter = params.kernel_diameter();
    let center = params.radius_cells as Real;
    ...
    for ((z, y, x), value) in weights.indexed_iter_mut() {
        ...
        let band_position = normalized_radius * band_count as Real;
        let band_index = band_position.floor() as usize;
        let local_radius = band_position.fract();
        let band_weight = bands[band_index.min(band_count.saturating_sub(1))];
        *value = band_weight * kernel_core_response(params.kernel_core, local_radius);
    }
...
fn kernel_core_response(kernel_core: KernelCore, radius: Real) -> Real {
    let r = radius.clamp(0.0, 1.0);
    match kernel_core {
        KernelCore::Polynomial => (4.0 * r * (1.0 - r)).powi(4),
...
mod tests {
    ...
    use crate::{KernelCore, KernelMode, KernelShell, LeniaParams};
    ...
    #[test]
    fn lenia_band_kernel_is_normalized() {
        let params = LeniaParams {
